...
<div
	class="table-shell mt-3 mb-2 max-w-full flex-1 gap-2 overflow-hidden rounded-2xl border border-gray-400/20 bg-base-100 p-3"
>
		...
		<table class={['table', 'table-pin-rows', 'w-full', zebra ? 'table-zebra' : '', className]}>
				...
				<tr class={[hover ? 'hover:bg-base-200' : '']}>
					{#if columns && !ths}
						{#each columns as column, columnIndex (column.id ?? `${column.title ?? 'column'}_${columnIndex}`)}
							<th
								class={[column.flex ? 'w-1/2' : '', column.className ? ' ' + column.className : '']}
			...
			<tbody>
				{#if rows && !trs}
					...
					{#if !rows.length}
						<tr>
							<td colspan={columns.length || 1} class="text-base-content/60">{emptyLabel}</td>
						</tr>
					{/if}
				{:else if trs}
					{@render trs()}
