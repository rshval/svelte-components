...
#### `InputPhone`: вставка номера и префиксы `+7` / `8`

Для `InputPhone` при активном выборе страны (`disabledCountry={false}`):

- вставка номера нормализуется (пробелы и разделители не ломают парсинг);
- если номер начинается с `+7` или с локальной `8` (11 цифр), префикс выносится в код страны;
- в поле номера остаётся только национальная часть (например, при вставке `+7 999 555 33 22` получится `999 555 33 22`).

Если выбор страны отключен (`disabledCountry={true}`), префиксы не извлекаются автоматически, чтобы не менять поведение фиксированного кода.
